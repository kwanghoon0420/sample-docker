...
- 컨트롤러나 Application Service 에서 if문구를 최대한 지양하고 필요한 판단로직이나 수행작업은 엔티티에게 위임합니다.
- 이렇게 분류한 이유는 도메인 별로 책임과 역할을 명확히 하고 컨트롤러나 Application Service 과 같은 루트 위치에서 봤을 때 이 요청은 어떤 작업을 하는지 한눈이 파악하기 쉽고 디테일한 작업을 알고싶을 때는 엔티티에 들어가서 확인할 수 있으며 특정 비즈니스 규칙이 변경될 때 해당 엔티티만 수정하면 되므로 유지보수성이 극대화됩니다.
